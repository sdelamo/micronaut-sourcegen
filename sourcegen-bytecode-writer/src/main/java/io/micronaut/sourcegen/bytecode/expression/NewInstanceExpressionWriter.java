...
import static io.micronaut.sourcegen.bytecode.WriterUtils.getConstructorDescriptor;

final class NewInstanceExpressionWriter implements ExpressionWriter {
    ...
    public void write(GeneratorAdapter generatorAdapter, MethodContext context, boolean statement) {
        ...
        for (ExpressionDef expression : newInstance.values()) {
            ExpressionWriter.pushExpression(generatorAdapter, context, expression, iterator.next());
        }
